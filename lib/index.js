...
function createAction(name) {
  var payloadCreator = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : identity_1;
  var metaCreator = arguments[2];

  invariant_1(isFunction_1(payloadCreator) || isNull_1(payloadCreator), 'Expected payloadCreator to be a function, undefined or null');

  var finalPayloadCreator = isNull_1(payloadCreator) || payloadCreator === identity_1 ? identity_1 : function (head) {
  ...
  var hasMeta = isFunction_1(metaCreator);
...
var reduceReducers = unwrapExports(lib);
...
function handleAction(symbol) {
  var reducer = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : identity_1;
  var defaultState = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

  var types = [symbol];
  invariant_1(!isUndefined_1(defaultState), 'defaultState for reducer handling ' + types.map(function (t) {
    return t.toString();
  }).join(', ') + ' should be defined');
  invariant_1(isFunction_1(reducer) || isPlainObject_1(reducer), 'Expected reducer to be a function or object with next and throw reducers');

  var _ref = isFunction_1(reducer) ? [reducer, reducer] : [reducer.next, reducer.throw].map(function (aReducer) {
    return isNil_1(aReducer) ? identity_1 : aReducer;
  }),
  ...
  return function () {
    ...
    if (!actionType || !includes_1(types, actionType)) {
      return state;
    }
...
function handleActions(handlers) {
  ...
  invariant_1(isPlainObject_1(handlers), 'Expected handlers to be an plain object.');
  ...
  var reducer = reduceReducers.apply(undefined, toConsumableArray(reducers));
...
function handleModule(moduleFile) {
  ...
  if (!reducers || reducers && !reducers.default) {
    missingPart += missingPart ? 'nor ' : '';
    missingPart += 'reducers';
  }
  if (missingPart) {
